...
const categories = ['hacks', 'supplements', 'adventures']
...
test('category chip has filled preset when it matches selected', () => {
  render(TagCloud, { categories, selected: 'hacks', show: true })
  expect(screen.getByText('hacks')).toHaveClass('preset-filled')
  expect(screen.getByText('All')).not.toHaveClass('preset-filled')
})

test('clicking a category chip fires selection change', async () => {
  const { component } = render(TagCloud, { categories, selected: 'all', show: true })
  await fireEvent.click(screen.getByText('hacks'))

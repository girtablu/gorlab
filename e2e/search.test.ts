...
test.describe("search", () => {
  ...
  test("submitting a query from the header navigates to the search page", async ({
    page,
  }) => {
    await page.goto(".");
    await page.getByRole("searchbox", { name: "Search" }).fill("basilisk");
  ...
  test("search page shows results for a known resource", async ({ page }) => {
    ...
    await expect(
      page.getByRole("link", { name: /basilisk/i }).first(),
  ...
  test("clicking a search result navigates to the resource page", async ({
    ...
    await page.goto("./search/?q=basilisk");
    await page
      .getByRole("link", { name: /basilisk/i })
      .first()
      .click();
  ...
  test("searching again from the search page updates results", async ({
    ...
    await page.getByRole("searchbox", { name: "Search" }).fill("vaarn");
    await page.getByLabel("Submit search").click();
    await expect(page).toHaveURL(/\/search\/\?q=vaarn/);
    await expect(
      page.getByRole("link", { name: /vaarn/i }).first(),
    ).toBeVisible({ timeout: 10000 });
  });
